  ...
  return `
    <div class="hit-miss ${tone}">
      <div class="hit-miss-head">
        ...
        <span class="badge ${tone}">${escapeHtml(verdict || "Prediction")}${pct != null ? ` · ${pct}%` : ""}</span>
      </div>
      ${pct != null ? `<div class="hit-meter"><span style="width:${pct}%"></span></div>` : ""}
    ...
    <section class="page page-v4">
      <div class="builder-layout thumbnail-layout">
        <form class="tool-card compact-form" id="thumbnail-form">
          ...
          <small class="form-hint">AI image uses Gemini image generation (needs a billing-enabled key). Falls back to the local canvas designer automatically.</small>
        </form>
...
function trendsOutputV4(data) {
  ...
  return `
    <article class="dashboard-card">
      ...
      ${normalizeList(data.topics).length ? insight("Topic ideas", normalizeList(data.topics).join(" · ")) : ""}
      ${normalizeList(data.audioTrends).length ? insight("Audio / sound trends (approximate)", normalizeList(data.audioTrends).join(" · ")) : ""}
      ${sources.length ? `
        <details class="source-list">
          <summary>Trending sources (${sources.length})</summary>
          <ul>${sources.map((v) => `<li>${escapeHtml(v.title || "")}${v.channel ? ` — ${escapeHtml(v.channel)}` : ""}${v.views ? ` · ${Number(v.views).toLocaleString()} views` : ""}</li>`).join("")}</ul>
        </details>` : ""}
    </article>
  `;
}
  ...
  return `
    <article class="dashboard-card">
      <div class="card-topline">
        ...
        <span class="badge ${tone}">${escapeHtml(result.verdict || "Reviewed")}${result.scoreOverall ? ` · ${result.scoreOverall}` : ""}</span>
      </div>
...
async function handleThumbnailImageV4(button) {
  await withBusy(button, "Generating...", async () => {
    ...
    if (!result.ok) {
      toast(result.data.message || result.data.error || "Image generation failed.");
      return;
    }
    if (!result.data.image?.data) {
      toast(result.data.note || "AI image unavailable. Using local designer.");
      generateThumbnailCanvas();
      return;
    }
